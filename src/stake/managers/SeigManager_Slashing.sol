    ...
    SeigManagerV1_3Storage
{
    ...
    modifier onlyRegistryOrOperator(address layer2) {
        require(
            msg.sender == _registry || msg.sender == Layer2I(layer2).operator(),
            'not onlyRegistryOrOperator'
        );
        _;
    }
    ...
    modifier checkCoinage(address layer2) {
        require(
            address(_coinages[layer2]) != address(0),
            'SeigManager: coinage has not been deployed yet'
        );
        _;
    }
    ...
    function setData(
        ...
        require(
            powerTONSeigRate + daoSeigRate + relativeSeigRate <= RAY,
            'exceeded seigniorage rate'
        );
        _powerton = powerton_;
        dao = daoAddress;
    ...
    function setPowerTONSeigRate(uint256 powerTONSeigRate_) external onlyOwner {
        require(
            powerTONSeigRate_ + daoSeigRate + relativeSeigRate <= RAY,
            'exceeded seigniorage rate'
        );
        powerTONSeigRate = powerTONSeigRate_;
        emit SetPowerTONSeigRate(powerTONSeigRate_);
    ...
    function setDaoSeigRate(uint256 daoSeigRate_) external onlyOwner {
        require(
            powerTONSeigRate + daoSeigRate_ + relativeSeigRate <= RAY,
            'exceeded seigniorage rate'
        );
        daoSeigRate = daoSeigRate_;
        emit SetDaoSeigRate(daoSeigRate_);
    ...
    function transferCoinageOwnership(
        address newSeigManager,
        address[] calldata coinages_
    ) external onlyOwner {
    ...
    ) external onlyRegistryOrOperator(layer2) returns (bool) {
    ...
    function slash(
        address layer2,
        address challenger
    ) external onlyChallenger checkCoinage(layer2) returns (bool) {
    ...
    function onWithdraw(
        ...
        require(
            _coinages[layer2].balanceOf(account) >= amount,
            'SeigManager: insufficiant balance to unstake'
        );
    ...
    function totalSupplyOfTon_2() external view returns (uint256 tos) {
        tos =
            ((ITON(_ton).totalSupply() -
                ITON(_ton).balanceOf(_wton) -
                ITON(_ton).balanceOf(address(0)) -
