...
const DAOProxyABI = require("../../abi/DAOCommitteeProxy.json").abi;
...
async function setOldDepositManagerAndMinter(deployer) {
    ...
    await (await daoCommitteeProxy.connect(daoCommitteeAdmin).upgradeTo(contractInfos.abis["DAOCommitteeExtend"].address)).wait()
    ...
    if(globalWithdrawalDelay.gt(ethers.constants.Zero)) {
        // console.log("1");
        // console.log("ethers.constants.Zero :", ethers.constants.Zero);
