#!/bin/bash
# =============================================================================
# Stop TON Staking V3 Local Devnet with Fast Withdrawal
# =============================================================================
# This script stops all services (L2, RAT clients, FW validators/aggregator)
# and optionally performs complete cleanup including FW config and BLS keys
# =============================================================================
...
echo -e "${BLUE}=== Stopping TON Staking V3 Local Devnet with Fast Withdrawal ===${NC}"
echo ""

# =============================================================================
# Step 1: Stop L2 + FW Containers
# =============================================================================
echo -e "${YELLOW}Step 1: Stopping L2 and Fast Withdrawal containers...${NC}"
...
echo -e "${GREEN}L2 and FW containers stopped${NC}"
echo ""
...
case $CLEANUP_OPTION in
    ...
    2|3)
        ...
        # Remove FW node config files and BLS key files (in case they remain)
        echo "  Removing FW node configs and BLS key files..."
        rm -f "$DEVNET_SEPOLIA_DIR"/fw-node-*.yaml 2>/dev/null || true
        rm -f "$DEVNET_SEPOLIA_DIR"/bls-keys-*.env 2>/dev/null || true
...
# Check containers
RUNNING_CONTAINERS=$(docker ps --filter name=ton-staking --format "{{.Names}}" | wc -l | tr -d ' ')
if [ "$RUNNING_CONTAINERS" -eq 0 ]; then
    echo -e "  ${GREEN}No ton-staking containers running${NC}"
else
    echo -e "  ${YELLOW}Warning: $RUNNING_CONTAINERS ton-staking containers still running${NC}"
fi
...
echo ""
echo -e "${GREEN}=== TON Staking V3 Local Devnet (with Fast Withdrawal) Stopped ===${NC}"
echo ""
