...
async function main() {
    ...
    let getTransactions = await MultiSigWallet.getTransaction(transactions);
    if(getTransactions.executed == false) {
        if(getTransactions.numConfirmations >= 2) {
            await MultiSigWallet.connect(signer).executeTransaction(transactions)
        } else {
            console.error("confirmTransaction 수가 부족합니다.");
            console.error("현재 confirmTransaction 수는 "+getTransactions.numConfirmations+"입니다.");
            process.exit(1);    
        }
    } else {
        console.error("이미 실행되었습니다.");
        process.exit(1);
    }
