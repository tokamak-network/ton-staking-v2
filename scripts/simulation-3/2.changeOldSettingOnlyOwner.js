...
async function changeDaoSetting() {
    ...
    //========================
    let candidateFactory_ = await daoCommitteeProxy.candidateFactory()
    ...
    let seigManager_ = await daoCommitteeProxy.seigManager()
    if (seigManager_.toLowerCase() != contractInfos.abis["SeigManagerProxy"].address.toLowerCase()) {
        await (await daoCommittee.connect(daoCommitteeAdmin).setSeigManager(
            contractInfos.abis["SeigManagerProxy"].address
            )).wait()
    }

    let layer2Registry_ = await daoCommitteeProxy.layer2Registry()
    if (layer2Registry_.toLowerCase() != contractInfos.abis["Layer2RegistryProxy"].address.toLowerCase()) {
    ...
    let seigManagerPfPwerTon = await powerTon.seigManager()
    if (seigManagerPfPwerTon.toLowerCase() != contractInfos.abis["SeigManagerProxy"].address.toLowerCase()) {
