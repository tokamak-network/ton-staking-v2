...
/**
 * @title DelegateStakingV3
 * @notice V3-integrated delegate staking for Tokamak Network
 * @dev Key Features:
 *      1. Integration with SeigManager V3 for eligibility checks
 *      2. Integration with OperatorManager for seigniorage claiming
 *      3. Auto-trigger mechanism for seigniorage distribution
 *      4. Emergency exit mechanism for L2 failure scenarios
 *      5. MasterChef-style reward distribution
 *
 * Architecture:
 ...
 */
contract DelegateStakingV3 is IDelegateStakingV3, ReentrancyGuard, Ownable {
    ...
    /// @inheritdoc IDelegateStakingV3
    function setAutoTrigger(bool enabled) external override {
        SequencerInfo storage info = sequencers[msg.sender];
        if (!info.isRegistered) revert SequencerNotRegistered();
    ...
    /// @inheritdoc IDelegateStakingV3
    function stake(address sequencer, uint256 amount) external override nonReentrant {
        if (amount == 0) revert ZeroAmount();
    ...
    /// @inheritdoc IDelegateStakingV3
    function unstake(address sequencer, uint256 amount) external override nonReentrant {
        if (amount == 0) revert ZeroAmount();
    ...
    /// @inheritdoc IDelegateStakingV3
    function triggerSeigniorage(address sequencer) external override nonReentrant {
    ...
    /// @inheritdoc IDelegateStakingV3
    function batchTriggerSeigniorage(address[] calldata _sequencerList) external override nonReentrant {
    ...
    function _claimFromOperatorManager(address operatorManager) internal returns (uint256 amount) {
        ...
        (bool success, ) = operatorManager.call(
            abi.encodeWithSignature("claimERC20(address,uint256)", address(wton), type(uint256).max)
        );
