...
describe("ClaimERC20 Check Test on Sepolia", () => {
    ...
    describe("DAOVault Check Tests", () => {
        it("should revert when trying to create agenda with claimTON", async () => {
            const noticePeriod = await daoagendaManager.minimumNoticePeriodSeconds();
            const votingPeriod = await daoagendaManager.minimumVotingPeriodSeconds();
            const agendaFee = await daoagendaManager.createAgendaFees();

            const target = [oldContractInfo.DAOVault];
            const functionBytecode = daovault.interface.encodeFunctionData(
                "claimTON", [user1.address,agendaFee])
            ...
            await expect(ton.connect(user1).approveAndCall(
                daoCommitteeProxy.address,
                agendaFee,
                param
            )).to.be.reverted;
        ...
        it("should revert when trying to create agenda with claimWTON", async () => {
            const noticePeriod = await daoagendaManager.minimumNoticePeriodSeconds();
            const votingPeriod = await daoagendaManager.minimumVotingPeriodSeconds();
            const agendaFee = await daoagendaManager.createAgendaFees();
            ...
            const target = [oldContractInfo.DAOVault];
            const functionBytecode = daovault.interface.encodeFunctionData(
                "claimWTON", [user1.address,agendaFee])
        ...
        it("should revert when trying to create agenda with claimERC20 (TON)", async () => {
            const noticePeriod = await daoagendaManager.minimumNoticePeriodSeconds();
            const votingPeriod = await daoagendaManager.minimumVotingPeriodSeconds();
            const agendaFee = await daoagendaManager.createAgendaFees();
            ...
            const target = [oldContractInfo.DAOVault];
            const functionBytecode = daovault.interface.encodeFunctionData(
                "claimERC20", [ton.address,user1.address,agendaFee])
        ...
        it("pass when trying to create agenda with claimERC20 (WTON)", async () => {
            const noticePeriod = await daoagendaManager.minimumNoticePeriodSeconds();
            const votingPeriod = await daoagendaManager.minimumVotingPeriodSeconds();
            const agendaFee = await daoagendaManager.createAgendaFees();
            ...
            const target = [oldContractInfo.DAOVault];
            const functionBytecode = daovault.interface.encodeFunctionData(
                "claimERC20", [wton.address,user1.address,agendaFee])
