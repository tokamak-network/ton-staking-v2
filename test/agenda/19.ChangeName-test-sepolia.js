...
describe("DAO Proxy Change Test", () => {
    ...
    describe("Add logic test", () => {
        it("1. setCandidateAddOnFactory test", async () => {
            await daoCommittee_Owner_Contract.connect(daoCommitteeAdmin).setCandidateAddOnFactory(
                layer2CandidateFactoryAddr
            )

            let Data = await daoCommittee_Owner_Contract.candidateAddOnFactory()
            expect(Data.toUpperCase()).to.be.equal(layer2CandidateFactoryAddr.toUpperCase())
        })

        it("2. setLayer2Manager test", async () => {
            await daoCommittee_Owner_Contract.connect(daoCommitteeAdmin).setLayer2Manager(
                layer2ManagerAddr
            )

            let Data = await daoCommittee_Owner_Contract.layer2Manager()
            expect(Data.toUpperCase()).to.be.equal(layer2ManagerAddr.toUpperCase())
        })
        ...
        it("4. setTargetSetL1BridgeRegistry  test", async () => {
            let beforeData = await seigManagerV3Contract.l1BridgeRegistry()

            await daoCommittee_Owner_Contract.connect(daoCommitteeAdmin).setTargetSetL1BridgeRegistry(
                seigManagerV3Contract.address,
                layer2CandidateFactoryAddr
            )

            let afterData = await seigManagerV3Contract.l1BridgeRegistry()
            expect(afterData.toUpperCase()).to.be.equal(layer2CandidateFactoryAddr.toUpperCase())
            ...
            let afterData2 = await seigManagerV3Contract.l1BridgeRegistry()
            expect(afterData2.toUpperCase()).to.be.equal(beforeData.toUpperCase())
        })
