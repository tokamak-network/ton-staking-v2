...
describe("currentAgendaStatus Test on Mainnet", () => {
    ...
    describe("memofiledGasFee Test", () => {
        it("1. Create Agenda (When the Memo field is not entered)", async () => {
            const noticePeriod = await daoagendaManager.minimumNoticePeriodSeconds();
            const votingPeriod = await daoagendaManager.minimumVotingPeriodSeconds();
            ...
            // create agenda
            let tx = await (await ton.connect(user1).approveAndCall(
                daoCommitteeProxy.address,
                agendaFee,
                param
            )).wait();

            console.log("When the Memo field is not entered")
        ...
        it("2. Create Agenda (Just a simple memo in the memo field)", async () => {
            const noticePeriod = await daoagendaManager.minimumNoticePeriodSeconds();
            const votingPeriod = await daoagendaManager.minimumVotingPeriodSeconds();
            ...
            console.log("Just a simple memo in the memo field")
            console.log(tx.cumulativeGasUsed);
            console.log(tx.effectiveGasPrice);
            console.log(tx.gasUsed);
            console.log("----------------------------------")
        ...
        it("3. Create Agenda (Long memo like URL)", async () => {
            const noticePeriod = await daoagendaManager.minimumNoticePeriodSeconds();
            const votingPeriod = await daoagendaManager.minimumVotingPeriodSeconds();
            ...
            memo = "https://snapshot.box/#/s:arbitrumfoundation.eth"
            ...
            console.log("Long memo like URL")
            console.log(tx.cumulativeGasUsed);
            console.log(tx.effectiveGasPrice);
            console.log(tx.gasUsed);
            console.log("----------------------------------")
