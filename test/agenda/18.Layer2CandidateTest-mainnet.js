...
describe("DAO Proxy Change Test", () => {
    ...
    describe("Setting another account & contract", () => {
        it("Deploy the TestLayer2", async () => {
            const Layer2 = await ethers.getContractFactory("TestLayer2");
            layer2Contract = await Layer2.deploy();

            await layer2Contract.deployed();
        })

        it("TestLayer2 setOperator", async () => {
            await layer2Contract.connect(user1).setOperator();
            let operator = await layer2Contract.operator();
            let isLayer2 = await layer2Contract.isLayer2();
            expect(operator).to.be.equal(user1.address)
            expect(isLayer2).to.be.equal(true)
        })
    ...
    describe("DAOCommittee_V1 Logic test", () => {
        ...
        it("registerLayer2CandidateByOwner Test", async () => {
            let beforeEnter = await daoCommittee_V1_Contract.isExistCandidate(layer2Contract.address)
            expect(beforeEnter).to.be.equal(false)
            await daoCommittee_V1_Contract.connect(daoCommitteeAdmin).registerLayer2CandidateByOwner(
                user1.address,
                layer2Contract.address,
                ""
            )

            let afterEnter = await daoCommittee_V1_Contract.isExistCandidate(layer2Contract.address)
            expect(afterEnter).to.be.equal(true)
        })
