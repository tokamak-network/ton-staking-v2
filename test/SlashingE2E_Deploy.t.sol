...
/**
 * @title SlashingE2E_Deploy
 * @notice E2E Slashing 테스트를 위한 전체 컨트랙트 배포 테스트
 * @dev docs/deployment-deployscript-slash.md 문서를 참조하여 작성
 */
contract SlashingE2E_Deploy is Test {
    // Tokens
    address public ton;
    address public wton;
    ...
    function _deployDepositManager() internal {
        ...
        console.log("DepositManager_setWithdrawalDelay deployed at:", address(depositManagerSetDelay));
        ...
        depositManagerProxy.setSelectorImplementations2(setWithdrawalDelaySelectors, address(depositManagerSetDelay));
        ...
        depositManagerProxy.setSelectorImplementations2(v1_SlashingSelectors, address(depositManagerV1_Slashing));
        console.log("DepositManagerV1_Slashing selectors routed");

        // Step 7: SlashingRewardRate 설정
        DepositManagerV1_Slashing(address(depositManagerProxy)).setSlashingRewardRate(SLASHING_REWARD_RATE);
    ...
    function _deployDAOCommittee() internal {
        ...
        DAOCommitteeProxy2(payable(daoCommitteeProxy)).setAliveImplementation2(address(daoCommitteeOwner), true);
        ...
        DAOCommitteeProxy2(payable(daoCommitteeProxy)).setSelectorImplementations2(ownerSelectors, address(daoCommitteeOwner));
        ...
        DAOCommitteeOwner(daoCommitteeProxy).setCandidateAddOnFactory(address(candidateAddOnFactoryProxy));
    ...
    function _deployOperatorManagerFactory() internal {
        ...
        console.log("OperatorManagerV1_1 implementation deployed at:", address(operatorManagerV1_1Impl));
    ...
    function test_DeploymentSuccessful() public view {
        ...
        assertTrue(address(l1BridgeRegistryProxy) != address(0), "L1BridgeRegistryProxy not deployed");
        assertTrue(address(operatorManagerFactory) != address(0), "OperatorManagerFactory not deployed");
    ...
    function test_SeigManagerConfiguration() public view {
        ...
        assertEq(seigManager.depositManager(), address(depositManagerProxy), "DepositManager address mismatch");
    ...
    function test_DepositManagerConfiguration() public view {
        ...
        assertEq(depositManager.registry(), address(layer2RegistryProxy), "Registry address mismatch");
        assertEq(depositManager.seigManager(), address(seigManagerProxy), "SeigManager address mismatch");
        assertEq(depositManager.globalWithdrawalDelay(), GLOBAL_WITHDRAWAL_DELAY, "GlobalWithdrawalDelay mismatch");
