...
contract SeigniorageFormulaValidation is Test {
    ...
    /// @notice 누구나 updateSeigniorage 호출 가능
    function test_anyoneCanCallUpdateSeigniorage() public {
    ...
    /// @notice V2 모드 (λ=1, r=0.4) 분배 테스트
    function test_v2ModeDistribution() public {
        // V3 마이그레이션 안 함 (V2 모드)
        seigManager.setV3Parameters(
            0.1e27,  // d = 10%
            0.1e27,  // θ = 10%
        ...
        uint256 totalSeig = seigManager.lastTotalSeig();
    ...
    /// @notice V3 완전 모드 (λ=0, r=0) 분배 테스트
    function test_v3FullModeDistribution() public {
        seigManager.setV3Parameters(
            0.1e27,  // d = 10%
            0.1e27,  // θ = 10%
            0.2e27,  // α = 20%
    ...
    /// @notice λ 감소에 따른 스테이커 시뇨리지 감소
    /// @dev λ=0일 때도 r > 0이면 S_relative가 남음
    ///      완전한 V3 모드(λ=0, r=0)에서만 staker seig = 0
    function test_transitionLambdaDecrease() public {
        seigManager.migrateToV3();
        seigManager.registerL2(layer2_1, 500e27, 100e27);
    ...
    /// @notice r 감소에 따른 V3 분배 재원 증가
    function test_transitionRDecrease() public {
        seigManager.migrateToV3();
        seigManager.registerL2(layer2_1, 500e27, 100e27);
    ...
    /// @notice 여러 L2가 Bridged TON 비례로 분배
    function test_multipleL2Distribution() public {
        seigManager.setV3Parameters(
            0.1e27,  // d = 10%
            0.1e27,  // θ = 10%
            0.2e27,  // α = 20%
    ...
    /// @notice 여러 번 연속 updateSeigniorage 호출
    ...
    /// @notice 같은 블록에서 중복 호출 시 추가 분배 없음
    function test_noDoubleDistributionSameBlock() public {
        seigManager.setV3Parameters(0.1e27, 0.1e27, 0.2e27, 500e27, 0, 0);
        seigManager.migrateToV3();
        seigManager.registerL2(layer2_1, 500e27, 100e27);
        ...
        seigManager.updateSeigniorage();
        uint256 firstDaoSeig = seigManager.lastDaoSeig();

        // 같은 블록에서 다시 호출
        seigManager.updateSeigniorage();
        uint256 secondDaoSeig = seigManager.lastDaoSeig();

        // 두 번째 호출에서는 span=0 이므로 분배 없음
        // (lastSeigBlock이 이미 업데이트되어 span=0)
        assertEq(secondDaoSeig, firstDaoSeig, "No additional distribution in same block");
    }
