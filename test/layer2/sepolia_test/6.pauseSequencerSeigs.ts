...
import L1BridgeRegistry_Json from '../../../artifacts/contracts/layer2/L1BridgeRegistryV1_1.sol/L1BridgeRegistryV1_1.json'
...
describe('L1BridgeRegistry', () => {
    ...
    const deployedL1BridgeRegistryAddress = "0xC8479A9F10a1E6275e0bFC4F9e058631fe63b8dC"
    ...
    before('create fixture loader', async () => {
        ...
        l1BridgeRegistry = new ethers.Contract(deployedL1BridgeRegistryAddress, L1BridgeRegistry_Json.abi,  deployer) as L1BridgeRegistryV1_1
    ...
    describe('# upgrade Logic SeigManagerV1_3', () => {
    ...
    describe('# pause the sequencer seigs', () => {

        it('setSeigniorageCommittee' , async () => {

            let seigniorageCommittee = await l1BridgeRegistry.seigniorageCommittee();
            // console.log('seigniorageCommittee', seigniorageCommittee);
            await (await l1BridgeRegistry.connect(tonHave).setSeigniorageCommittee(deployer.address)).wait()

            seigniorageCommittee = await l1BridgeRegistry.seigniorageCommittee();
            // console.log('seigniorageCommittee', seigniorageCommittee);
        })

        it('pause the sequencer(titan-sepolia) seigs' , async () => {
            await l1BridgeRegistry.connect(deployer).rejectCandidateAddOn(deployedLegacySystemConfigAddress)
        })
