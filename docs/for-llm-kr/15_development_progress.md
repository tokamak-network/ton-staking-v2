...
| 항목 | 설명 | 관련 문서 |
|-----|------|---------|
| 검증자 보상 분배 방식 | 해석 1 (균등) vs 해석 2 (L2별) | `12_tbd_items.md` §2.1 |
| 검증자 파라미터 값 | `c_m`, `C_off`, `D_min` 결정 | `12_tbd_items.md` §2.4 |
| 검증자 담보금 시뇨리지 정책 | 귀속처 및 처리 방식 | `12_tbd_items.md` §3.3 |

### 6.2 중간 우선순위

| 항목 | 설명 | 관련 문서 |
|-----|------|---------|
| 몰수된 담보금 귀속처 | DAO/소각/분배 중 선택 | `12_tbd_items.md` §2.3 |
| RAT 증거 형식 | Optimism 방식 채택 여부 | `12_tbd_items.md` §3.1 |
| 슬래싱 금액의 시뇨리지 처리 | 몰수 시 포함 여부 | `12_tbd_items.md` §3.2 |
| 검증자 출금 시 가스비 정책 | 부담 주체 결정 | `12_tbd_items.md` §3.4 |
