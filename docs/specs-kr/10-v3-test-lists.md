...
- **중복 테스트 삭제**: V2V3ModeSwitching.t.sol에서 SM-029, SM-030, SM-031, SM-032 제거
  - SM-029: SM-001과 기능 중복 (V2 선형 시뇨리지)
  - SM-030, SM-031, SM-032: SeigManagerPausable.t.sol의 SM-050, SM-054, SM-063과 기능 중복
- **문서 누락 테스트 추가**: SM-033~SM-039 (이미 코드에 존재했으나 문서에 누락)
- **_excludeLayer2FromSeigniorage / _includeLayer2ToSeigniorage 수정**:
  - V2/V3 모드 분리: V2에서는 layer2RewardInfo만, V3에서는 bridgedTONInfo만 처리
- **SEC-010, SEC-012 삭제**: 불필요한 NOTE 주석 제거
- **SeigniorageFormulaValidation.t.sol 삭제**: Mock 컨트랙트 기반 테스트 제거
  - 모든 시나리오가 SeigniorageAccuracy.t.sol, MultiL2SeigniorageDistribution.t.sol 등 실제 컨트랙트 테스트로 커버됨
