...
- `_accStaked[layer2][account]`: 계정별 L2 스테이킹 금액
- `_accStakedLayer2[layer2]`: L2별 총 스테이킹 금액
- `_pendingUnstaked[layer2][account]`: 대기 중인 출금 요청
...
| **시퀀서 등록** | 담보금 예치 및 등록 |
| **담보금 관리** | 추가 예치, 조회 |
| **슬래싱 처리** | Fraud 발생 시 담보금 슬래싱 |
| **챌린저 보상** | 슬래싱된 금액에서 챌린저 보상 |
| **담보금 조회** | SeigManager에 담보금 정보 제공 |
...
│  입력:                                                                   │
│  ┌───────────────────┐                                                  │
│  │ 누구나            │──► registerSequencer(), addDeposit()            │
│  │                   │──► slashSequencerByGame() (Permissionless)      │
│  │ OperatorManager  │──► deactivateSequencer() (출금)                  │
│  └───────────────────┘                                                  │
│                                                                          │
│  조회:                                                                   │
│  ┌─────────────────┐                                                    │
│  │ Layer2Manager  │◄── layer2 → systemConfig 매핑                      │
│  │ L1BridgeRegistry│◄── DisputeGameFactory 검증                        │
│  │ FaultDisputeGame│◄── 게임 상태 확인                                  │
│  └─────────────────┘                                                    │
│                                                                          │
│  조회 제공:                                                              │
│  ┌─────────────────┐                                                    │
│  │ SeigManager    │◄── getSequencerDepositByLayer2()                  │
│  └─────────────────┘                                                    │
│                                                                          │
│  출력:                                                                   │
│  ┌─────────────────┐                                                    │
│  │ OperatorManager│──► TON 반환 (탈퇴 시)                               │
│  │ 챌린저          │──► TON 보상                                        │
│  │ DAO Treasury   │──► 슬래싱 잔여분                                    │
│  └─────────────────┘                                                    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
- `sequencerDeposits[systemConfig]`: 시퀀서 담보금 정보
- `layer2ToSystemConfig[layer2]`: Layer2 → SystemConfig 매핑
- `minimumStakingRatio` (θ): 최소 스테이킹 비율
- `maxFraudProofCost` (C_max): 최대 Fraud Proof 비용
- `maxChallengers` (H_max): 최대 챌린저 수
- `accumulatedSlashings`: 누적 슬래싱 금액
- `challengerPendingRewards[challenger]`: 챌린저 미청구 보상
...
│  검증자 흐름:                                                                     │
│  ┌──────────┐     등록      ┌───────────────┐   정보제공   ┌───────────────┐  │
│  │ 검증자    │─────────────►│     RAT       │────────────►│ValidatorReward│  │
│  └──────────┘              └───────────────┘             └───────────────┘  │
│                                                                                  │
│  시퀀서 흐름:                                                                     │
│  ┌──────────┐     등록      ┌───────────────┐   담보금조회  ┌──────────────┐  │
│  │ 시퀀서    │─────────────►│SequencerVault │────────────►│ SeigManager  │  │
│  └──────────┘              └───────────────┘             └──────────────┘  │
│                                                                                  │
│  Bridged TON 흐름:                                                              │
│  ┌──────────────┐   알림    ┌──────────────┐    조회     ┌─────────────────┐ │
