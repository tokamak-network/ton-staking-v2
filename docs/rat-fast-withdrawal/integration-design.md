...
- ✅ **RollupType 3**: OPTIMISM_BEDROCK_WITH_DISPUTE_GAME
  - DisputeGame 기반 출금
  - OutputRootProof 검증 필요
  - 기존 RAT Client Type3 로직 재사용
...
│   ├── validator/
│   │   ├── RAT.sol                         # ⭐ BLS 공개키 등록 기능 추가
│   │   ├── RATStorage.sol
│   │   └── libraries/
│   │       └── Type3EvidenceVerifier.sol
│   └── withdrawal/                         # 🆕 Fast Withdrawal Contracts
│       ├── FastWithdrawal.sol              # 메인 컨트랙트
│       └── libraries/
...
#### 3.1.1 RAT Contract 수정

**파일**: `src/validator/RAT.sol`
...
struct ValidatorRegistration {
    ...
    bool isActive;
    bytes blsPublicKey;             // ⭐ NEW: BLS 공개키 (48 bytes)
}
```
...
#### 3.1.2 FastWithdrawal Contract (신규)

**파일**: `src/withdrawal/FastWithdrawal.sol` (신규 생성)
